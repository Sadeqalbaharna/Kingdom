...
class ArmorsTab extends StatelessWidget {
  ...
  Future<void> _add(BuildContext context) async {
    ...
    final res = await showDialog<InsurancePolicy>(context: context, builder: (_)=> AlertDialog(title: const Text('Add Magical Armor'), content: Column(mainAxisSize: MainAxisSize.min, children:[
    ...
    ]), actions:[TextButton(onPressed: ()=> Navigator.pop(context), child: const Text('Cancel')), FilledButton(onPressed: (){ final p=int.tryParse(premium.text)??0; final c=int.tryParse(coverage.text)??0; Navigator.pop(context, InsurancePolicy(id: DateTime.now().millisecondsSinceEpoch.toString(), name: name.text.isEmpty? 'Policy': name.text, premium: p, coverage: c, active: true)); }, child: const Text('Add'))]));
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(padding: const EdgeInsets.all(16), children: [
      Row(children: [const Icon(Icons.security_outlined), const SizedBox(width:8), const Text('Magical Armors (Insurance)', style: TextStyle(fontWeight: FontWeight.w600)), const Spacer(), FilledButton.icon(onPressed: ()=> _add(context), icon: const Icon(Icons.add), label: const Text('Add'))]),
      const SizedBox(height: 8),
      ...items.map((p)=> Card(margin: const EdgeInsets.only(top:8), child: ListTile(
        title: Text(p.name),
        subtitle: Text('Premium: ${fmtInt(p.premium)} • Coverage: ${fmtInt(p.coverage)}'),
        leading: Switch(value: p.active, onChanged: (v)=> ctrl.setArmorActive(p.id, v)),
        trailing: IconButton(icon: const Icon(Icons.edit), onPressed: () async {
          ...
          final ok = await showDialog<bool>(context: context, builder: (_)=> AlertDialog(title: const Text('Edit Armor'), content: Column(mainAxisSize: MainAxisSize.min, children:[
          ...
          if (ok==true) ctrl.editArmor(p.id, name: n.text, premium: int.tryParse(pr.text), coverage: int.tryParse(co.text));
        }),
      )))
    ]);
  }
